...
namespace YAX
{
    ...
    {
        if (_goingFullscreen && !glfwGetWindowMonitor(_handle))
        {
            ...
            //Just create another window as fullscreen and share the context,
            //then scrap the old one
            GLFWwindow* win = glfwCreateWindow(_width, _height, _title.c_str(), dest, _handle);
            glfwDestroyWindow(_handle);
            _handle = win;
        }
        else if (glfwGetWindowMonitor(_handle) && !_goingFullscreen)
        {
            //Create a non-fullscreen window and share the context,
            //then scrap the old one
            
            GLFWwindow* win = glfwCreateWindow(_width, _height, _title.c_str(), NULL, _handle);
            glfwDestroyWindow(_handle);
            _handle = win;
        }
    }
